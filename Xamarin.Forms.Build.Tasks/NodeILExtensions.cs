	...
	static class NodeILExtensions
	{
		...
		public static bool CanConvertValue(this ValueNode node, ILContext context, TypeReference targetTypeRef, TypeReference typeConverter)
		{
			...
			if (typeConverter != null && str != null) {
				var typeConvAttribute = typeConverter.GetCustomAttribute(module.GetOrImportReference(typeof(TypeConversionAttribute)));
			...
			bool boxValueTypes, bool unboxValueTypes)
		{
			...
			var compiledConverterName = typeConverter?.GetCustomAttribute (module.GetOrImportReference(typeof(ProvideCompiledAttribute)))?.ConstructorArguments?.First().Value as string;
			...
			if (compiledConverterName != null && (compiledConverterType = Type.GetType (compiledConverterName)) != null) {
				...
				if (targetTypeRef.IsValueType && boxValueTypes)
					yield return Instruction.Create (OpCodes.Box, module.GetOrImportReference (targetTypeRef));
				yield break;
			}
			...
			{
				var isExtendedConverter = typeConverter.ImplementsInterface(module.GetOrImportReference(typeof (IExtendedTypeConverter)));
				var typeConverterCtor = typeConverter.Resolve().Methods.Single(md => md.IsConstructor && md.Parameters.Count == 0 && !md.IsStatic);
				var typeConverterCtorRef = module.GetOrImportReference(typeConverterCtor);
				var convertFromInvariantStringDefinition = isExtendedConverter
					? module.GetOrImportReference(typeof (IExtendedTypeConverter))
						.Resolve()
				...
				var convertFromInvariantStringReference = module.GetOrImportReference(convertFromInvariantStringDefinition);
				...
				if (targetTypeRef.IsValueType && unboxValueTypes)
					yield return Instruction.Create(OpCodes.Unbox_Any, module.GetOrImportReference(targetTypeRef));
			...
			if (targetTypeRef.Resolve().BaseType != null && targetTypeRef.Resolve().BaseType.FullName == "System.Enum")
				...
				var ticks = ts.Ticks;
				var timeSpanCtor =
					module.GetOrImportReference(typeof(TimeSpan))
						.Resolve()
						.Methods.FirstOrDefault(md => md.IsConstructor && md.Parameters.Count == 1);
				var timeSpanCtorRef = module.GetOrImportReference(timeSpanCtor);
				...
				var ticks = dt.Ticks;
				var dateTimeCtor =
					module.GetOrImportReference(typeof(DateTime))
						.Resolve()
						.Methods.FirstOrDefault(md => md.IsConstructor && md.Parameters.Count == 1);
				var dateTimeCtorRef = module.GetOrImportReference(dateTimeCtor);
				...
				if (decimal.TryParse(str, NumberStyles.Number, CultureInfo.InvariantCulture, out outdecimal)) {
					var vardef = new VariableDefinition(context.Body.Method.Module.GetOrImportReference(typeof(decimal)));
					...
					var getInvariantInfo =
						context.Body.Method.Module.GetOrImportReference(typeof(CultureInfo))
							.Resolve()
							.Properties.FirstOrDefault(pd => pd.Name == "InvariantCulture")
							.GetMethod;
					var getInvariant = context.Body.Method.Module.GetOrImportReference(getInvariantInfo);
					yield return Instruction.Create(OpCodes.Call, getInvariant);
					yield return Instruction.Create(OpCodes.Ldloca, vardef);
					var tryParseInfo =
						context.Body.Method.Module.GetOrImportReference(typeof(decimal))
							.Resolve()
							.Methods.FirstOrDefault(md => md.Name == "TryParse" && md.Parameters.Count == 4);
					var tryParse = context.Body.Method.Module.GetOrImportReference(tryParseInfo);
					...
					var decimalctor = context.Body.Method.Module.GetOrImportReference(decimalctorinfo);
			...
			if (isNullable)
				yield return Instruction.Create(OpCodes.Newobj, module.GetOrImportReference(nullableCtor));
			if (originalTypeRef.IsValueType && boxValueTypes)
				yield return Instruction.Create(OpCodes.Box, module.GetOrImportReference(originalTypeRef));
		}
		...
		public static IEnumerable<Instruction> PushXmlLineInfo(this INode node, ILContext context)
		{
			...
			if (xmlLineInfo.HasLineInfo()) {
				...
				ctor = module.GetOrImportReference(typeof(XmlLineInfo).GetConstructor(new[] { typeof(int), typeof(int) }));
			}
			else
				ctor = module.GetOrImportReference(typeof(XmlLineInfo).GetConstructor(new Type[] { }));
		...
		public static IEnumerable<Instruction> PushParentObjectsArray(this INode node, ILContext context)
		{
			...
			var finalArray = new VariableDefinition(module.GetOrImportReference(typeof (object[])));
			...
			if (context.ParentContextValues != null)
			{
				...
				var arrayCopy =
					module.GetOrImportReference(typeof (Array))
						.Resolve()
						.Methods.First(
							md =>
								md.Name == "Copy" && md.Parameters.Count == 5 &&
								md.Parameters[1].ParameterType.FullName == module.TypeSystem.Int32.FullName);
				yield return Instruction.Create(OpCodes.Call, module.GetOrImportReference(arrayCopy));
			}
			...
			if (nodes.Count > 0)
			{
				for (var i = 0; i < nodes.Count; i++)
				{
					...
					if (context.Variables[en].VariableType.IsValueType)
						yield return Instruction.Create(OpCodes.Box, module.GetOrImportReference(context.Variables[en].VariableType));
...
//				IL_000f:  callvirt instance class [mscorlib] System.Reflection.PropertyInfo class [mscorlib] System.Type::GetProperty(string)
				...
				var getPropertyInfo = module.GetOrImportReference(typeof(Type).GetMethod("GetProperty", new [] { typeof(string) }));
				yield return Instruction.Create(OpCodes.Ldtoken, module.GetOrImportReference(declaringTypeReference ?? propertyRef.DeclaringType));
				yield return Instruction.Create(OpCodes.Call, module.GetOrImportReference(getTypeFromHandle));
				yield return Instruction.Create(OpCodes.Ldstr, propertyRef.Name);
				yield return Instruction.Create(OpCodes.Callvirt, module.GetOrImportReference(getPropertyInfo));
				yield break;
			}
			yield return Instruction.Create(OpCodes.Ldnull);
			yield break;
		...
		public static IEnumerable<Instruction> PushServiceProvider(this INode node, ILContext context, FieldReference bpRef = null, PropertyReference propertyRef = null, TypeReference declaringTypeReference = null)
		{
			...
			var addService = module.GetOrImportReference(addServiceInfo);

			var getTypeFromHandle =
				module.GetOrImportReference(typeof(Type).GetMethod("GetTypeFromHandle", new[] { typeof(RuntimeTypeHandle) }));
			...
			if (pushParentIl[pushParentIl.Count - 1].OpCode != OpCodes.Ldnull)
			{
				...
				yield return Instruction.Create(OpCodes.Ldtoken, module.GetOrImportReference(typeof (IProvideValueTarget)));
				yield return Instruction.Create(OpCodes.Call, module.GetOrImportReference(getTypeFromHandle));
				...
				var targetProviderCtor =
					module.GetOrImportReference(typeof (SimpleValueTargetProvider).GetConstructor(new[] { typeof (object[]), typeof(object) }));
			...
			if (context.Scopes.ContainsKey(node))
			{
				...
				yield return Instruction.Create(OpCodes.Ldtoken, module.GetOrImportReference(typeof (INameScopeProvider)));
				yield return Instruction.Create(OpCodes.Call, module.GetOrImportReference(getTypeFromHandle));
				var namescopeProviderCtor = module.GetOrImportReference(typeof (NameScopeProvider).GetConstructor(new Type[] { }));
				...
				var setNamescope = module.GetOrImportReference(typeof (NameScopeProvider).GetProperty("NameScope").GetSetMethod());
			...
			if (node.NamespaceResolver != null)
			{
				...
				yield return Create(Ldtoken, module.GetOrImportReference(typeof (IXamlTypeResolver)));
				yield return Create(Call, module.GetOrImportReference(getTypeFromHandle));
				var xmlNamespaceResolverCtor = module.GetOrImportReference(typeof (XmlNamespaceResolver).GetConstructor(new Type[] { }));
				var addNamespace = module.GetOrImportReference(typeof (XmlNamespaceResolver).GetMethod("Add"));
				...
				var xtr = module.GetOrImportReference(typeof (XamlTypeResolver)).Resolve();
				var xamlTypeResolverCtor = module.GetOrImportReference(xtr.Methods.First(md => md.IsConstructor && md.Parameters.Count == 2));
			...
			{
				yield return Instruction.Create(OpCodes.Dup); //Dupicate the serviceProvider
				yield return Instruction.Create(OpCodes.Ldtoken, module.GetOrImportReference(typeof (IXmlLineInfoProvider)));
				yield return Instruction.Create(OpCodes.Call, module.GetOrImportReference(getTypeFromHandle));

				foreach (var instruction in node.PushXmlLineInfo(context))
					yield return instruction;

				var lip = module.GetOrImportReference(typeof (XmlLineInfoProvider)).Resolve();
				var lineInfoProviderCtor = module.GetOrImportReference(lip.Methods.First(md => md.IsConstructor && md.Parameters.Count == 1));
